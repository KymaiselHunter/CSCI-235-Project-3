...
*/
template<class ItemType>
void ArrayBag<ItemType>::operator/=(const ArrayBag &pArrayBag)
{
   for(int i = 0; i < pArrayBag.getCurrentSize(); i++)
   {
      if(!this->contains(pArrayBag.items_[i]))
      {
         this->add(pArrayBag.items_[i]);
      }
   }
}
